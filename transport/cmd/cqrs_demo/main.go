...
func main() {
	...
	log.Println("Starting CQRS Demo Application...")
	...
	businessTransportService := business.NewTransportService(
		commandBus,
	...
	// API 핸들러 초기화 (JSON-RPC)
	rpcHandler := api.NewBatchRPCHandler(businessTransportService)

	// HTTP 라우터 설정
	router := mux.NewRouter()

	// 기존 REST API 엔드포인트
	router.HandleFunc("/api/transports", transportService.CreateTransportHandler).Methods("POST")
	router.HandleFunc("/api/transports/{id}/join", transportService.JoinTransportHandler).Methods("POST")
	router.HandleFunc("/api/transports/{id}/start", transportService.StartTransportHandler).Methods("POST")
	router.HandleFunc("/api/transports/{id}", transportService.GetTransportHandler).Methods("GET")
	router.HandleFunc("/api/transports", transportService.GetActiveTransportsHandler).Methods("GET")
	router.HandleFunc("/api/transports/{id}/raid", transportService.RaidTransportHandler).Methods("POST")
	router.HandleFunc("/api/transports/{id}/defend", transportService.DefendTransportHandler).Methods("POST")
