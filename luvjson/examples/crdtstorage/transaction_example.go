...
func main() {
	...
	var (
		...
		distributed = flag.Bool("distributed", true, "분산 트랜잭션 활성화 여부")
	)
	flag.Parse()
	...
	options.KeyPrefix = fmt.Sprintf("tx-%s", peerID)
	options.EnableDistributedLock = *distributed
	options.DistributedLockTimeout = time.Minute
	options.EnableTransactionTracking = *distributed
	...
	// 사용자 입력 처리
	go handleUserInputs(ctx, doc, *distributed)
...
func printItems(doc *crdtstorage.Document) {
	...
	// 아이템 목록 가져오기
	itemsMap, ok := content.(map[string]interface{})
	if !ok {
		fmt.Println("문서 내용이 맵 형식이 아닙니다.")
		return
	}

	items, ok := itemsMap["items"].([]interface{})
	if !ok {
...
func handleUserInputs(ctx context.Context, doc *crdtstorage.Document, distributed bool) {
	for {
		fmt.Println("\n명령어:")
		fmt.Println("1. 아이템 추가")
		fmt.Println("2. 아이템 수정")
		fmt.Println("3. 아이템 삭제")
		fmt.Println("4. 아이템 목록 보기")
		fmt.Println("5. 동시 편집 시뮬레이션")
		fmt.Println("6. 종료")
		fmt.Print("> ")
		...
		switch choice {
		case 1:
			addItem(ctx, doc, distributed)
		case 2:
			updateItem(ctx, doc, distributed)
		case 3:
			deleteItem(ctx, doc, distributed)
		case 4:
			printItems(doc)
		case 5:
			simulateConcurrentEdits(ctx, doc, distributed)
		case 6:
			return
		default:
...
func addItem(ctx context.Context, doc *crdtstorage.Document, distributed bool) {
	// 아이템 정보 입력
	var item GameItem
	...
	item.Attributes = strings.Split(attributesStr, ",")
	...
	if distributed {
		// 분산 트랜잭션 사용
		txResult := doc.EditTransaction(ctx, func(api *api.ModelApi) error {
			// 현재 내용 가져오기
		...
		fmt.Println("아이템이 추가되었습니다. (분산 트랜잭션 사용)")
	} else {
		// 일반 편집 사용
		editResult := doc.Edit(ctx, func(api *api.ModelApi) error {
			// 현재 내용 가져오기
			content, err := api.View()
			if err != nil {
				return fmt.Errorf("failed to get content: %w", err)
		...
		fmt.Println("아이템이 추가되었습니다.")
	}
}

// updateItem은 기존 아이템을 수정합니다.
func updateItem(ctx context.Context, doc *crdtstorage.Document, distributed bool) {
	// 문서 내용 가져오기
	content, err := doc.GetContent()
	if err != nil {
		fmt.Printf("문서 내용 가져오기 실패: %v\n", err)
	...
	case 4:
		...
		var value string
		fmt.Scanln(&value)
		fieldValue = strings.Split(value, ",")
	case 5:
		fieldName = "owner"
		fmt.Print("새 소유자: ")
		var value string
	...
	if distributed {
		...
		fmt.Println("아이템이 수정되었습니다. (분산 트랜잭션 사용)")
	} else {
		// 일반 편집 사용
		editResult := doc.Edit(ctx, func(api *api.ModelApi) error {
			// 현재 내용 가져오기
...
func deleteItem(ctx context.Context, doc *crdtstorage.Document, distributed bool) {
	// 문서 내용 가져오기
	content, err := doc.GetContent()
	if err != nil {
		fmt.Printf("문서 내용 가져오기 실패: %v\n", err)
	...
	if distributed {
		...
		fmt.Println("아이템이 삭제되었습니다. (분산 트랜잭션 사용)")
	} else {
		// 일반 편집 사용
		editResult := doc.Edit(ctx, func(api *api.ModelApi) error {
			// 현재 내용 가져오기
...
func simulateConcurrentEdits(ctx context.Context, doc *crdtstorage.Document, distributed bool) {
	fmt.Print("동시 편집 수: ")
	var count int
	fmt.Scanln(&count)
	if count <= 0 {
	...
	for i := 0; i < count; i++ {
		go func(index int) {
			...
			item := GameItem{
				...
				Attributes:  []string{"동시", "편집", "테스트", strconv.Itoa(index)},
			...
			if distributed {
				...
				if !txResult.Success {
					...
					fmt.Printf("동시 편집 %d 성공. (분산 트랜잭션 사용)\n", index)
				}
			} else {
				// 일반 편집 사용
				editResult := doc.Edit(ctx, func(api *api.ModelApi) error {
